...
func TestGenerateFont_Complete(t *testing.T) {
	...
	t.Run("Check dimensions", func(t *testing.T) {
		for char, lines := range font {
			if len(lines) != 8 {
				t.Errorf(
					"Character %c has %d lines, expected 8",
					char,
					len(lines),
				)
			}
			...
			for i, line := range lines {
				if len(line) != 8 {
					t.Errorf(
						"Character %c line %d has length %d, expected 8",
						char,
						i,
						len(line),
					)
	...
	t.Run("Space character should be all spaces", func(t *testing.T) {
		...
		for i, line := range space {
			for _, ch := range line {
				if ch != ' ' {
					t.Errorf(
						"Space char line %d contains non-space character: %c",
						i,
						ch,
					)
				}
...
func TestGenerateFont_VowelsSpecial(t *testing.T) {
	...
	for _, vowel := range vowels {
		...
		if !isDifferent {
			t.Errorf(
				"Vowel %c is identical to consonant B",
				vowel,
			)
		}
	}
